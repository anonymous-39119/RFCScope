...
from main.analyze_rfcs import main as analyze

parser = argparse.ArgumentParser(
    description="Analyze an RFC"
)
parser.add_argument(
    "rfc_number",
    type=str,
    help="The RFC number of the RFC to analyze"
)
parser.add_argument(
    "output_directory",
    type=str,
...
analyze(
    ...
    section_dependency_graph_path=os.path.join(args.output_directory, 'section_dependencies.json'),
    corpus_path=os.path.join(args.output_directory, 'corpus.json'),
    error_type=0,
    output_dir=os.path.join(results_path, 'inconsistencies')
)
print(f"Inconsistencies analysis completed. Results saved to {os.path.join(results_path, 'inconsistencies')}.")
...
print(f"Underspecifications analysis completed. Results saved to {os.path.join(results_path, 'underspecifications')}.")
