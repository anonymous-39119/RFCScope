...
from retrieval.rfc_retriever import __get_first_int
...
@cache
def get_external_reference_summary(document_tag, reference_value, reference_type, url, reference_text):
    """
    ...
    """
    document_title = reference_text + f"({document_tag})" if reference_text else document_tag
...
def build_section_dependency_graph(corpus, references_information):
    ...
    if 'structured_content' in rfc:
        process_sections_for_graph(rfc_number, rfc['structured_content'], graph, [], references_information)
    ...
    for i, edge in enumerate(graph):
        for j, other_edge in enumerate(graph):
            if i < j and all(edge[p] == other_edge[p] for p in [
            "source", 
            "document_type", 
            "document_tag", 
            "reference_type", 
    ...
    for i, edge in enumerate(graph):
        if (
            edge["source"]["rfc_number"] == edge["document_tag"] and
            edge["source"]["section_id"] == edge["reference_value"]
        ):
...
def process_sections_for_graph(rfc_number, sections, graph, updates, references):
    ...
    for section_id, section in sections.items():        
                ...
                if 'references' in chunk and chunk['references']:
                    for ref in chunk['references']:
                        ...
                        for reference in references:
                            if all(p in reference["tag"] for p in [str(document_tag), "RFC"]):
        ...
        if 'children' in section:
            process_sections_for_graph(rfc_number, section['children'], graph, updates, references)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Generate a section dependency graph from an RFC corpus')
    parser.add_argument('corpus_path', type=str, help='Path to the corpus JSON file')
    parser.add_argument('references_path', type=str, help='Path to the RFC references JSON file')
    parser.add_argument('--output', '-o', type=str, help='Output directory path for saving the output graph JSON file')
