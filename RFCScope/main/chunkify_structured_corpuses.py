...
from retrieval.references import get_references_from_rfc_excerpt

MIN_PARA_LINES = 3 # minimum number of lines in a paragraph to be considered a paragraph
...
def split_content_into_chunks(rfc_number: str, content: str, perform_reference_extraction: bool = False, references_information: dict = None) -> list[dict]:
    """
    ...
    for paragraph in tqdm(paragraphs, desc="Splitting content into chunks"):
        if perform_reference_extraction:
            references, referenced_texts = get_references_from_rfc_excerpt(rfc_number, paragraph, references_information)
...
def process_sections(rfc_number: str, sections: dict, perform_reference_extraction: bool = False, references: dict = None) -> dict:
    """
    ...
    for section_id, section in sections.items():
        ...
        if 'content' in section:
            decomposed_content = split_content_into_chunks(rfc_number, section['content'], perform_reference_extraction, references)
            if "decomposed_content" not in section:
                ...
                for i, chunk in enumerate(decomposed_content):
                    ...
                    section["decomposed_content"][i]["references"] = chunk["references"] if chunk["references"] else []
                    section["decomposed_content"][i]["external_references"] = chunk["external_references"] if chunk["external_references"] else []

        if 'children' in section:
            process_sections(rfc_number, section['children'], perform_reference_extraction, references)
    
    return sections


def process_corpus(input_path: str, output_path: str, perform_reference_extraction: bool = False, references: dict = None):
    """
    ...
    if 'structured_content' in rfc:
        process_sections(rfc['rfc_number'], rfc['structured_content'], perform_reference_extraction, references)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Process RFC corpus to decompose section contents')
    parser.add_argument('input', type=str, help='Path to the input corpus JSON file')
    parser.add_argument('--references', type=str, help='Path to the RFC references JSON file')
