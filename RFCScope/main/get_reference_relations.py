...
from retrieval.rfc_retriever import get_structured_rfc
...
def get_referenced_rfcs(rfc_number: int) -> list[dict]:
    ...
    for _, section_data in rfc_contents.items():
        if section_data['title'].lower() in ["references", "normative references", "informative references"]:
    ...
    if not reference_sections:
        ...
        for _, section_data in rfc_contents.items():
            reference_section = __extract_reference_section(section_data['content'], "References") or reference_section
            normative_reference_section = __extract_reference_section(section_data['content'], "Normative References") or normative_reference_section
            informative_reference_section = __extract_reference_section(section_data['content'], "Informative References") or informative_reference_section

        reference_sections = [x for x in [reference_section, normative_reference_section, informative_reference_section] if x is not None]
    ...
    for references_section in reference_sections:
        ...
        # "Normative References" or "Informative References"
            ...
            references_section['children'][child_id]['title'].lower() in ["normative references", "informative references"]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "rfc_number",
        type=str,
        help="RFC number to extract references from"
    )
    parser.add_argument(
        "output_directory",
        type=str,
