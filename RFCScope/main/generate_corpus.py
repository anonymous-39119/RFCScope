...
import json
import argparse
from retrieval.rfc_retriever import get_structured_rfc, get_rfc_title
...
def __get_section_contents(section_id: str, section_data: dict) -> str:
    ...
    return "\n".join([
        ...
        "",
        *[__get_section_contents(child, section_data['children'][child]) for child in section_data['children']],
        ""
    ])
...
def __should_skip_section(section_data: dict, skip_references: bool, skip_considerations: bool, skip_legacy_sections: bool) -> bool:
    """
    Unified function to determine if a section should be skipped based on the given options.
    """
    if skip_references and (
        section_data['title'].lower().startswith("references") or 
        section_data['title'].lower() in ["normative references", "informative references"]
    ):
...
def get_rfc_contents(
    ...
    * If `skip_legacy_sections` is True, sections like "Acknowledgements", "Authors'
    ...
    for section_id, section_data in rfc_contents.items():
        if not __should_skip_section(section_data, skip_references, skip_considerations, skip_legacy_sections):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Generate RFC corpus from dependency graph')
    parser.add_argument('rfc_number', type=int, help='RFC number to generate corpus for')
    parser.add_argument('output', type=str, help='Path to the output directory for saving the corpus')
