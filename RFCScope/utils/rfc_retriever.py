...
def process_rfc_html(html: str, clean_legacy_sections: bool = True) -> dict:
    ...
    for heading in all_headings:
        ...
        if (
            selflink
            and selflink.next_sibling
            and selflink.next_sibling.text[:3] == " - "
        ):
    ...
    for section in sections_array:
        ...
        for other_section in sections_array:
            ...
            if (
                all(x == y for x, y in zip(identifier, other_identifier))
                and (
                    parent is None
                    or len(other_identifier) > len(parent["identifier"].split("."))
                )
                and len(other_identifier) < len(identifier)
            ):
...
def get_structured_rfc(
    rfc_number: int | str, clean_legacy_sections: bool = True
) -> dict:
    """
