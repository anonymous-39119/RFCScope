...
from utils.rfc_retriever import get_structured_rfc, __get_first_int
...
def is_reference_from_rfc(
    reference: dict, reference_information: list[dict]
) -> None | str:
    """
    ...
    if reference_information:
        matching_refs = [
            ...
            if ref["tag"] == reference["document_tag"]
        ]
        if matching_refs:
...
def get_references(
    text: str, reference_information: list[dict] = None
) -> list[dict[str, str]]:
    """
...
def __find_section(sections: dict, section_identifier: str) -> dict:
    ...
    for section in sections:
        found_section = __find_section(
            sections[section]["children"], section_identifier
        )
        if found_section:
...
def __process_section_reference(
    reference: dict, rfc_number: str, reference_rfc: dict
) -> dict[str, dict[str, dict]]:
    # first try to perform an exact search for the 'reference_value' in the RFC section identifiers
    choices = [
        section["identifier"].strip()
        for section in __get_flat_sections_list(reference_rfc)
    ]
    ...
    if reference["reference_value"] in choices:
        ...
        selected_section = process.extractOne(
            reference["reference_value"], choices, scorer=fuzz.ratio
        )[0]
    ...
    return {
        rfc_number: {
            selected_section: {"title": section["title"], "content": section["content"]}
        }
    }


@cache
def __process_text_reference(
    reference: dict, rfc_number: str, reference_rfc: dict
) -> dict[str, dict[str, dict]]:
...
def collect_all_reference_texts(
...
def get_references_from_text(
    text: str,
) -> tuple[list[dict[str, str]], dict[str, dict[str, dict]]]:
    """
...
def __search_for_section_references(excerpt_text: str) -> list[str]:
    ...
    for ref in section_refs:
        ...
        refs.append(
            f"Section {section_identifier} of RFC <PLEASE FIGURE OUT THE RFC NUMBER>"
        )

    return refs


def get_references_from_rfc_excerpt(
    rfc_number: str, excerpt_text: str, references: dict = None
) -> tuple[list[dict[str, str]], dict[str, dict[str, dict]]]:
    """
    ...
    reference_tags_present = list(
        filter(lambda ref: ref in excerpt_text, reference_tags)
    )
    ...
    prompt += (
        ...
        + "\n".join([f"* {ref}" for ref in references_to_look_for])
    )
...
def get_references_from_erratum(
    rfc_number: str, erratum_text: str
) -> tuple[list[dict[str, str]], dict[str, dict[str, dict]]]:
    """
    ...
    """
    refs = get_references(
        f"Consider the following erratum for RFC {rfc_number}.\n\n```\n{erratum_text}\n```"
    )
    ref_texts = collect_all_reference_texts(refs)
    return refs, ref_texts
